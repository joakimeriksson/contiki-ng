...
## Current Port Status (October 12, 2025)
...
- LEDs (user LED on P0.00)
- Buttons (user button on P0.18)
- OpenOCD flashing via CMSIS-DAP
- hello-world example

### ⏳ Pending Features:
- 802.15.4 radio driver (stub in place)
- IPv6 networking stack (disabled until radio works)
- Low-power modes
...
```c
// LEDs
#define LEDS_CONF_RED    0  // P0.00 - User LED (red)

// Buttons
#define BUTTON_USER_PIN  18 // P0.18 - User button
#define BUTTON_USER_PORT 0

// UART (USB CDC)
// Handled by onboard SAMD11 chip
```
...
The nRF54L15 has multiple GRTC (Global Real-Time Counter) instances. This port uses **GRTC_0** for both clock and rtimer:

- **MDK defines:** `GRTC_IRQn = GRTC_0_IRQn`
- **nrfx HAL redefines:** `GRTC_IRQn = GRTC_2_IRQn` for application core
- **Our workaround:** Undef MDK definition in `nrfx_glue.h`, use explicit `GRTC_0_IRQn` in source

See `arch/cpu/nrf/nrf54l15/clock-arch.c` and `rtimer-arch.c` for implementation.
...
- `gpio_hal_arch_interrupt_enable_nrf54l15()`
- `gpio_hal_arch_interrupt_disable_nrf54l15()`

These automatically select the correct GPIOTE instance based on the pin's port.

### Networking Disabled

The radio driver for nRF54L15's 802.15.4 peripheral is not yet implemented. A stub driver is used:
- File: `arch/cpu/nrf/nrf54l15/nrf-ieee-driver-stub.c`
- Returns error codes for all operations
- Allows build without networking enabled

Configuration in `nrf54l15-conf.h`:
```c
...
- **Solution:** Networking is disabled - use hello-world or non-networking examples
...
- **Check:** Pin P0.00 is correctly defined in board config
- **Try:** Use the `leds-example` to test LED functionality

## Known Limitations

1. **No Radio:** 802.15.4 radio driver not implemented yet
2. **No Networking:** IPv6 stack disabled (requires radio)
3. **No Low-Power:** Low-power modes not tested/optimized
4. **OpenOCD Required:** Standard OpenOCD releases lack nRF54L15 support
5. **Single Board:** Only XIAO variant tested (DK port would be similar)

## Development Roadmap

### Phase 1: Basic Functionality ✅ (Completed Oct 12, 2025)
- [x] Build system
- [x] GRTC timers
- [x] GPIO/GPIOTE
- [x] UART console
...
### Phase 2: Radio Driver (In Progress)
- [ ] Study nRF54L15 Radio peripheral differences from nRF52/53
- [ ] Adapt existing nRF radio driver
- [ ] Test TX/RX functionality
- [ ] 802.15.4 compliance testing
...
**Status:** Alpha - Basic functionality working, radio pending
